...
typedef struct AsepriteDataFile {
    Str fullname;
    u8slice content;
} AsepriteDataFile;
typedef struct DataFiles {AsepriteDataFile* ptr; i64 len;} DataFiles;

typedef struct PlatformShader {
    Str label;
    u8slice content;
} PlatformShader;
typedef struct PlatformShaders {PlatformShader* ptr; i64 len;} PlatformShaders;

typedef struct Platform {
    void (*writeEntireFile)(Arena* arena, Str filename, void* data, i64 size);
    void (*executeCommandLine)(Str command);
} Platform;

void build(Arena* arena, DataFiles* dataFiles, PlatformShaders* platformShaders, Str exename, Platform platform) {

    AssetStructBuilder assetStructBuilder_ = {.strb = arenaAllocDynarr(arena, char, 20 * Megabyte)};
    AssetStructBuilder* assetStructBuilder = &assetStructBuilder_;

    append(&assetStructBuilder->strb, "// generated by build program, do not edit by hand\n\n");
    ...
    for (i64 index = 0; index < dataFiles->len; index++) {
        AsepriteDataFile file = dataFiles->ptr[index];

        assert(strends(file.fullname, STR(".aseprite")));
        AseFile* ase = (AseFile*)file.content.ptr;
        assert(ase->magic == 0xA5E0);

        Str filename = strslice(file.fullname, 0, file.fullname.len - (sizeof(".aseprite") - 1));
        ...
        if (isStage) {
            ...
            FileInfoEntity info = {.content = ase, .names.file = file.fullname};
    ...
    {
        strbuilderEnumBegin(assetStructBuilder, STR("EntityID"));
        for (i64 ind = 0; ind < entityNamesDedup.len; ind++) {strbuilderEnumAdd(assetStructBuilder, entityNamesDedup.ptr[ind]);}
        strbuilderEnumAdd(assetStructBuilder, STR("Count"));
        strbuilderEnumEnd(assetStructBuilder);
    }

    {
        strbuilderEnumBegin(assetStructBuilder, STR("StageID"));
        ...
        for (i64 ind = 0; ind < fileInfosStages.len; ind++) {
            ...
            if (!*alreadyDone) {
                *alreadyDone = true;
                ...
                strbuilderEnumAdd(assetStructBuilder, name);
            }
        }
        strbuilderEnumAdd(assetStructBuilder, STR("Count"));
        strbuilderEnumEnd(assetStructBuilder);
    ...
    strbuilderEnumBegin(assetStructBuilder, STR("AnimationID"));
    for (i64 ind = 0; ind < fileInfosEntities.len; ind++) {
        FileInfoEntity info = fileInfosEntities.ptr[ind];
        Str animationName = strfmt(arena, "%.*s_%.*s", LIT(info.names.entity), LIT(info.names.animation));
        strbuilderEnumAdd(assetStructBuilder, animationName);
        animationNames.ptr[ind] = animationName;
    }
    strbuilderEnumAdd(assetStructBuilder, STR("Count"));
    strbuilderEnumEnd(assetStructBuilder);

    strbuilderEnumBegin(assetStructBuilder, STR("AtlasID"));
    strbuilderEnumAdd(assetStructBuilder, STR("Whitepx"));
    strbuilderEnumAdd(assetStructBuilder, STR("Font"));
    for (i64 ind = 0; ind < fileInfosEntities.len; ind++) {
        FileInfoEntity info = fileInfosEntities.ptr[ind];
        ...
        for (int frameIndex = 0; frameIndex < info.content->frameCount; frameIndex++) {
            ...
            strbuilderEnumAdd(assetStructBuilder, atlasName);
        }
    }
    for (i64 ind = 0; ind < fileInfosStages.len; ind++) {
        FileInfoStage info = fileInfosStages.ptr[ind];
    ...
    i32 totalAtlasTextureCount = assetStructBuilder->addCount;
    strbuilderEnumAdd(assetStructBuilder, STR("Count"));
    strbuilderEnumEnd(assetStructBuilder);

    strbuilderTableBegin(assetStructBuilder, STR("int"), STR("globalFirstAtlasIDEntities"), STR("EntityID_Count"));
    {
        Str currentEntity = {};
        for (i64 ind = 0; ind < fileInfosEntities.len; ind++) {
            FileInfoEntity info = fileInfosEntities.ptr[ind];
            if (!streq(currentEntity, info.names.entity)) {
                ...
                builderTableAdd(assetStructBuilder, key, value);
                currentEntity = info.names.entity;
            }
        }
    }
    strbuilderTableEnd(assetStructBuilder);

    strbuilderTableBegin(assetStructBuilder, STR("int"), STR("globalFirstAtlasIDStages"), STR("StageID_Count"));
    {
    ...
    strbuilderTableBegin(assetStructBuilder, STR("int"), STR("globalAnimationCumulativeFrameCounts"), STR("AnimationID_Count"));
    {
    ...
    strbuilderTableBegin(assetStructBuilder, STR("int"), STR("globalStageVariantCounts"), STR("StageID_Count"));
    {
    ...
    for (i32 fileInfoIndex = 0; fileInfoIndex < fileInfosStages.len; fileInfoIndex++) {
        ...
        if (false) {
            ...
            platform.writeEntireFile(arena, STR("temp.txt"), tempStr - tempStrPitch - 1, tempStrLen);
        }
    ...
    strbuilderEnumBegin(assetStructBuilder, STR("ShaderID"));
    struct {u8slice* ptr; i32 len, cap;} shaders = arenaAllocDynarr(arena, u8slice, platformShaders->len);
    struct {u8* ptr; i64 len, cap;} allShaderData = arenaAllocDynarr(arena, u8, 50 * Megabyte);
    for (i32 shaderIndex = 0; shaderIndex < platformShaders->len; shaderIndex++) {
        PlatformShader platformShader = platformShaders->ptr[shaderIndex];
        u8slice shaderDataOg = platformShader.content;
        ...
        strbuilderEnumAdd(assetStructBuilder, platformShader.label);
    }
    strbuilderEnumAdd(assetStructBuilder, STR("Count"));
    strbuilderEnumEnd(assetStructBuilder);

    BinBuilder binb = arenaAllocDynarr(arena, u8, 50 * Megabyte);
    AssetDataBuilder datab_ = {.bin = &binb, .str = assetStructBuilder, .ptrfixes = arenaAllocDynarr(arena, PtrFix, 1024)};
    ...
    platform.writeEntireFile(arena, STR("code/generated.c"), assetStructBuilder->strb.ptr, assetStructBuilder->strb.len);
